...
(function () {
  ...
  const state = {
    step: 0,
    llmProvider: null, // 'gemini' | 'claude-code'
  ...
  // Screens are: welcome → provider → apikey? → speech → whisper? → finish
  // apikey is only visited when llmProvider === 'gemini'; whisper/model-download
  // only when speechProvider === 'whisper'.
  const stepScreens = ['welcome', 'provider', 'apikey', 'speech'];
  ...
  // and picking Claude Code skips the Gemini key screen entirely.
  function computeScreenOrder() {
    const out = ['welcome', 'provider'];
    if (state.llmProvider !== 'claude-code') out.push('apikey');
    out.push('speech');
  ...
  // ── Wire up: AI Provider choices ──────────────────────────────────
  function setStatusPill(el, state_, text) {
  ...
  const claudeDetectRow = $('#claudeDetectRow');
  const claudeDetectCmd = $('#claudeDetectCmd');
  const claudeDetectStatus = $('#claudeDetectStatus');
  const claudeInstallHint = $('#claudeInstallHint');

  async function runClaudeDetect() {
    claudeDetectRow.style.display = 'flex';
    claudeInstallHint.style.display = 'none';
    claudeDetectCmd.textContent = 'scanning…';
    setStatusPill(claudeDetectStatus, 'testing', 'Probing');
    try {
      const r = await window.electronAPI.detectClaudeCli();
      if (r.found) {
        state.claudeCliDetected = true;
        claudeDetectCmd.textContent = r.version ? `${r.command} (v${r.version})` : r.command;
        setStatusPill(claudeDetectStatus, 'success', 'Found');
      } else {
        state.claudeCliDetected = false;
        claudeDetectCmd.textContent = 'not found';
        setStatusPill(claudeDetectStatus, 'error', 'Not installed');
        claudeInstallHint.style.display = 'block';
      }
    } catch (e) {
      state.claudeCliDetected = false;
      setStatusPill(claudeDetectStatus, 'error', 'Probe failed');
      claudeInstallHint.style.display = 'block';
    }
  }

  $$('#providerChoices .choice-card').forEach((card) => {
    card.addEventListener('click', () => {
      const value = card.dataset.value;
      state.llmProvider = value;
      $$('#providerChoices .choice-card').forEach((c) => c.classList.remove('selected'));
      card.classList.add('selected');
      if (value === 'claude-code') {
        runClaudeDetect();
      } else {
        claudeDetectRow.style.display = 'none';
        claudeInstallHint.style.display = 'none';
      }
    });
  });
  ...
  function populateSummary() {
    const rows = [];
    if (state.llmProvider === 'claude-code') {
      rows.push({
        label: '<i class="fas fa-robot"></i> AI Provider',
        value: state.claudeCliDetected ? 'Claude Code (detected)' : 'Claude Code (not detected)',
        cls: state.claudeCliDetected ? 'ok' : 'skip',
      });
    } else {
      rows.push({
  ...
  nextBtn.addEventListener('click', async () => {
    ...
    // Persist the AI provider choice immediately so it's saved even if
    // the user closes the wizard before reaching Finish.
    if (name === 'provider' && window.electronAPI) {
      try {
        await window.electronAPI.saveSettings({ llmProvider: state.llmProvider });
