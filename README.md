...
The setup script writes sensible defaults. With the default `gemini` provider, the only required value is a Gemini API key. Switching to `claude-code` needs no API key, just the CLI installed and logged in.

```bash
# Optional. Pick one: gemini (default) or claude-code
LLM_PROVIDER=gemini

# Required for the gemini provider
GEMINI_API_KEY=your_gemini_api_key_here

# Optional, used when LLM_PROVIDER=claude-code
# Requires the Claude Code CLI installed and logged in: https://claude.com/code
# No API key needed — it reuses your existing Claude Code login. Screenshot
# analysis always uses Gemini regardless of this setting.
...
2. **Reason.** Gemini or Claude Code (your choice in Settings) reads the audio or text with full conversation context and works toward a precise answer. Screenshots always go through Gemini.
...
- Claude Code CLI as an alternate text/voice backend, switchable from Settings
