...
class LLMService {
  ...
  /** Read live (not cached at construction) so a provider switch in Settings takes effect immediately. */
  _useClaudeCode() {
    return String(process.env.LLM_PROVIDER || config.get('llm.provider') || 'gemini').trim().toLowerCase() === 'claude-code';
  }
  ...
  async processImageWithSkill(imageBuffer, mimeType, activeSkill, sessionMemory = [], programmingLanguage = null) {
    if (this._useClaudeCode()) {
      return require('./claude-code.service').processImageWithSkill();
    }
  ...
  async processImageWithSkillStream(imageBuffer, mimeType, activeSkill, sessionMemory = [], programmingLanguage = null, onDelta = null) {
    if (this._useClaudeCode()) {
      return require('./claude-code.service').processImageWithSkillStream();
    }
  ...
  async processTextWithSkillStream(text, activeSkill, sessionMemory = [], programmingLanguage = null, onDelta = null) {
    if (this._useClaudeCode()) {
      return require('./claude-code.service').processTextWithSkillStream(text, activeSkill, sessionMemory, programmingLanguage, onDelta);
    }
...
Remember: Be intelligent about filtering - only provide detailed responses when the user actually needs help with ${activeSkill}.`;
  ...
  async processTranscriptionWithIntelligentResponseStream(text, activeSkill, sessionMemory = [], programmingLanguage = null, onDelta = null) {
    if (this._useClaudeCode()) {
      return require('./claude-code.service').processTranscriptionWithIntelligentResponseStream(text, activeSkill, sessionMemory, programmingLanguage, onDelta);
    }
  ...
  async testConnection() {
    if (this._useClaudeCode()) {
      return require('./claude-code.service').testConnection();
    }
  ...
  getStats() {
    if (this._useClaudeCode()) {
      return require('./claude-code.service').getStats();
    }
