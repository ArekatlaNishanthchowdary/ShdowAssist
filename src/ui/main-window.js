...
class MainWindowUI {
    constructor() {
        ...
        this.isHidden = false;
        this.currentSkill = 'all'; // Default, will be updated from settings
        this.statusDot = null;
    ...
    setupElements() {
        ...
        this.skillIndicator = document.getElementById('skillSelector') || document.getElementById('skillIndicator');
        this.skillSelect = document.getElementById('activeSkill');
        this.settingsIndicator = document.getElementById('settingsButton') || document.getElementById('settingsIndicator'); // Optional
        ...
        if (!this.statusDot || (!this.skillIndicator && !this.skillSelect) || !this.micButton || !this.screenshotButton) {
        ...
        // Chat button click handler toggles the chat window
        if (this.chatButton) {
            this.chatButton.addEventListener('click', () => {
                if (this.isInteractive && window.electronAPI && window.electronAPI.switchToChat) {
                    window.electronAPI.switchToChat();
                }
            });
        }

        // Skill select dropdown change handler
        if (this.skillSelect) {
            this.skillSelect.addEventListener('change', (e) => {
                const newSkill = e.target.value;
                this.currentSkill = newSkill;
                if (window.electronAPI && window.electronAPI.saveSettings) {
                    window.electronAPI.saveSettings({ activeSkill: newSkill });
                }
            });
        }

        // Settings button click handler opens settings window directly
        if (this.settingsIndicator) {
    ...
    setupKeyboardShortcuts() {
        document.addEventListener('keydown', (e) => {
            ...
            // Handle Ctrl + Arrow keys based on interaction mode
            if (e.ctrlKey && ['ArrowUp', 'ArrowDown', 'ArrowLeft', 'ArrowRight'].includes(e.key)) {
                e.preventDefault();

                if (this.isInteractive) {
                    // Interactive mode: Ctrl + Up/Down for skill navigation
                    if (e.key === 'ArrowUp') {
                    ...
                    // Non-interactive mode: Ctrl + Arrow keys for window movement
    ...
    navigateSkill(direction) {
        if (!this.availableSkills || this.availableSkills.length === 0) return;
        const currentIndex = this.availableSkills.indexOf(this.currentSkill);
        let nextIndex = (currentIndex + direction) % this.availableSkills.length;
        if (nextIndex < 0) nextIndex += this.availableSkills.length;
    ...
    updateSkillIndicator() {
        ...
        const skillName = skillNames[this.currentSkill] || (this.currentSkill ? this.currentSkill.toUpperCase() : 'DSA');
        if (this.skillIndicator) {
    ...
    showSettingsMenu() {
        ...
        const quitOption = this.createMenuItem('Quit ShadowAssist', 'fa-power-off', () => {
