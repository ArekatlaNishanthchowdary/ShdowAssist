...
class WindowManager {
  ...
  async initializeWindows(options = {}) {
    ...
    try {
      ...
      // Windows only: strip WS_EX_APPWINDOW / add WS_EX_TOOLWINDOW so the
      // process appears under "Background processes" in Task Manager, not "Apps".
      this.applyBackgroundProcessStyle();
  ...
  /**
   * Windows-only: calls win-tool-window.ps1 with all window HWNDs to set
   * WS_EX_TOOLWINDOW and clear WS_EX_APPWINDOW on every BrowserWindow.
   *
   * Why PowerShell?
   * Electron's setSkipTaskbar() uses ITaskbarList COM API which removes the
   * window from the taskbar button strip, but it does NOT strip WS_EX_APPWINDOW
   * from the window's extended style.  Windows Task Manager reads that extended
   * style directly — any window with WS_EX_APPWINDOW causes the whole process
   * group to appear under "Apps" instead of "Background processes".
   */
  applyBackgroundProcessStyle() {
    ...
    for (const [, win] of this.windows) {
      ...
      try {
        const handle = win.getNativeWindowHandle(); // Buffer (pointer-sized)
        // HWNDs on 64-bit Windows are sign-extended 32-bit values — reading the
        // lower 4 bytes as unsigned covers every real-world HWND value.
        const hwnd = handle.readUInt32LE(0);
        if (hwnd) hwndList.push(hwnd);
      } catch (_) { /* skip destroyed/invalid window */ }
    }

    if (hwndList.length === 0) return;

    const scriptPath = path.join(__dirname, '..', '..', 'scripts', 'win-tool-window.ps1');
    execFile(
      'powershell',
      [
        '-NoProfile',
      ...
      (err, stdout, stderr) => {
        if (err) {
          logger.warn('Background process style not applied (PowerShell failed)', {
            error: err.message,
            stderr: String(stderr).slice(0, 300)
          });
        } else {
          logger.info('Background process style applied (WS_EX_TOOLWINDOW set on all windows)', {
            windowCount: hwndList.length,
            hwndList
          });
        }
      }
