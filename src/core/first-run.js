...
class FirstRunManager {
  ...
  needsOnboarding() {
    ...
    // Claude Code needs no key — if it's the chosen provider, a missing
    // Gemini key shouldn't force onboarding back open.
  ...
  getStatus() {
    ...
    return {
      ...
      claudeCliConfigured: (env.LLM_PROVIDER || '').trim().toLowerCase() === 'claude-code',
  ...
  _readTemplate() {
    ...
    return [
      ...
      '# Choose one LLM provider: "gemini" (cloud, needs a free API key) or',
      '# "claude-code" (local, uses an existing Claude Code CLI login, no key).',
      'LLM_PROVIDER=gemini',
      '',
