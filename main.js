...
// ── Windows: classify process as a Background Process in Task Manager ──
// Without this, Windows Shell sees any Electron process that creates a
// BrowserWindow as a foreground "App". Setting the AppUserModelId to the
// raw executable path (instead of a friendly product name) opts the process
// out of the shell's "Apps" bucket and keeps it in "Background processes".
// This must happen before app.whenReady() — setting it later has no effect.
...
const claudeCodeService = require("./src/services/claude-code.service");
...
class ApplicationController {
  ...
  async onAppReady() {
    ...
    // Re-enforce background process classification on Windows.
    // Windows may re-evaluate the process category after the event loop
    // settles; calling this again here keeps it pinned to Background processes.
    if (process.platform === "win32") {
      app.setAppUserModelId(process.execPath);
    }
  ...
  ipcMain.handle("capture-area", (event, options) => captureService.captureAndProcess(options));
    ...
    ipcMain.handle("resize-window", (event, { width, height }) => {
      ...
      if (mainWindow) {
        ...
        const minW = 60;
        const maxW = windowManager.windowConfigs?.main?.width || 700;
    ...
    // Detect an installed Claude Code CLI (used when LLM_PROVIDER=claude-code).
    ipcMain.handle("detect-claude-cli", async () => {
      try {
        return claudeCodeService.detect();
      } catch (e) {
        logger.warn("Claude Code CLI detection failed", { error: e.message });
        return { found: false, command: null, version: null, error: e.message };
      }
    });
  ...
  const capture = await captureService.captureAndProcess();
      ...
      if (!capture.imageBuffer || !capture.imageBuffer.length) {
        this.showLLMErrorResponse("Failed to capture screenshot image", { skill: this.activeSkill });
      ...
      this.showLLMErrorResponse(error.message, { skill: this.activeSkill });
  ...
  // ponytail: no HTML window listens to the ocr-error/llm-error IPC events above,
  // so a failed request used to just hide the loading state and go silent.
  // Reuse the existing response window/markdown renderer to actually show the error.
  showLLMErrorResponse(errorMessage, metadata = {}) {
    windowManager.showLLMResponse(`⚠️ ${errorMessage}`, { ...metadata, isError: true });
  }
  ...
  getSettings() {
    ...
    return {
      ...
      claudeCliConfigured: claudeCodeService.isInitialized,
  ...
  saveSettings(settings) {
    try {
      ...
      if (settings.llmProvider === "gemini" || settings.llmProvider === "claude-code") {
        envUpdates.LLM_PROVIDER = settings.llmProvider;
      }
      if (settings.claudeCliCommand !== undefined) {
        envUpdates.CLAUDE_CLI_COMMAND = settings.claudeCliCommand;
      }
      ...
      const prevClaudeCliCommand = process.env.CLAUDE_CLI_COMMAND || '';
      ...
      // Reinitialize the Claude Code CLI probe when its command changes or
      // it's newly selected as the LLM provider, so claudeCliConfigured in
      // getSettings() (and the Settings/onboarding status pill) reflects
      // reality without needing an app restart.
      const claudeCliCommandChanged = envUpdates.CLAUDE_CLI_COMMAND !== undefined &&
        prevClaudeCliCommand !== String(settings.claudeCliCommand || '');
      if (envUpdates.LLM_PROVIDER === 'claude-code' || claudeCliCommandChanged) {
        try {
          claudeCodeService.initializeClient();
          logger.info('Claude Code service reinitialized after settings change', {
            claudeCliCommandChanged
          });
        } catch (e) {
          logger.warn('Failed to reinitialize Claude Code service after settings change', {
            error: e.message
          });
        }
      }
